...
type ColumnType = ColumnDef<QuizInterface, unknown>;
...
export function QuizListPage(props: QuizListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getQuizzes({
        relations: ['organization', 'participant.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<QuizInterface>>(
    () => `/quizzes?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: QuizInterface) => {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('organization', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('participant', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'participant',
          header: 'Participant',
          accessorKey: 'participant',
          cell: ({ row: { original: record } }: any) => record?._count?.participant,
        }
      : null,

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/quizzes/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
